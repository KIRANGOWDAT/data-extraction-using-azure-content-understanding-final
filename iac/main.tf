...
module "app_service" {
  ...
  application_insights = {
    name                  = lower(local.application_insights_name)
...
resource "azurerm_role_assignment" "function_app_key_vault_secrets" {
  scope                = module.key_vault.output.resource_id
...
resource "azurerm_role_assignment" "current_user_storage_blob" {
  ...
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = data.azurerm_client_config.current.object_id
  description          = "Current user access to Storage Blob"
}

# Role assignment for Log Analytics workspace read access
resource "azurerm_role_assignment" "current_user_log_analytics_reader" {
  scope                = module.log_analytics_workspace.output.resource_id
  role_definition_name = "Log Analytics Reader"
  principal_id         = data.azurerm_client_config.current.object_id
  description          = "Current user read access to Log Analytics workspace for App Insights"
}
